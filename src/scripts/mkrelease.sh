...
sed -i "s/${PREV_VERSION}/${APP_VERSION}/g" hemppay.pro > /dev/null
...
$QT_STATIC/bin/qmake hemppay.pro -spec linux-clang CONFIG+=release > /dev/null
echo "[OK]"
...
cat hemppay.pro | sed "s/precompile_header/release/g" | sed "s/PRECOMPILED_HEADER.*//g" > zec-qt-wallet-mingw.pro
echo "[OK]"
